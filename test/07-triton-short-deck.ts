...
import { CardGroup, HandEquity, HandRank,
         OddsCalculator, ShortDeckRank, TritonShortDeckGame } from '../src';

describe('OddsCalculator: triton short-deck', () => {
  it('no board', () => {
    const player1Cards: CardGroup = CardGroup.fromString('AcAh');
    const player2Cards: CardGroup = CardGroup.fromString('JdTd');
    const result: OddsCalculator = OddsCalculator.calculate([player1Cards, player2Cards], undefined, 'triton', 10000);
  ...
  it('calculates proper odds', () => {
    ...
    let result: OddsCalculator = OddsCalculator.calculate([player1Cards, player2Cards], board, 'triton', 10000);
    ...
    result = OddsCalculator.calculate([player1Cards, player2Cards], board, 'triton', 500);
  ...
  it('flush beats fullhouse', () => {
    ...
    const result: OddsCalculator = OddsCalculator.calculate([player1Cards, player2Cards], board, 'triton', 1);
  ...
  it('a-6-7-8-9 is a straight', () => {
    const game: TritonShortDeckGame = new TritonShortDeckGame();
  ...
  it('throws error when card lower than 6 given', () => {
    ...
    expect(OddsCalculator.calculate.bind(null, [player1Cards, player2Cards], null, 'triton'))
    ...
    expect(OddsCalculator.calculate.bind(null, [player1Cards, player2Cards], board, 'triton'))
      .to.throw(Error, 'Only cards rank 6 through A are valid');
  });
  ...
  it('straight beat trips', () => {
    ...
    const oddsPlayer1: number = result.equities[0].getEquity();
    const oddsPlayer2: number = result.equities[1].getEquity();
    expect(oddsPlayer1).to.be.equal(0);
    expect(oddsPlayer2).to.be.equal(100);
  });
});
